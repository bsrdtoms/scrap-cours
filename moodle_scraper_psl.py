...
# -*- coding: utf-8 -*-
"""
Moodle Offline Scraper — PSL (moodle.psl.eu)
=============================================
Variante du scraper ENSAI adaptée pour le Moodle PSL.
Authentification via navigateur (Shibboleth / Microsoft OAuth) :
  le script ouvre Chrome, tu te connectes manuellement,
  puis le scraping continue automatiquement.
...
# ============================================================
MOODLE_URL   = "https://moodle.psl.eu"
OUTPUT_DIR   = Path(__file__).parent / "moodle_offline_psl"
LOG_FILE     = Path(__file__).parent / "moodle_scraper_psl.log"
...
# ============================================================
class MoodleScraper:
    def __init__(self, username=None, password=None, output_dir=OUTPUT_DIR,
    ...
    def login(self):
        """
        Connexion à Moodle PSL.
        Ouvre Chrome pour que tu te connectes manuellement (Shibboleth/Microsoft),
        puis transfère les cookies vers requests.
        """
        login_url = f"{self.base_url}/login/index.php"
        log.info("Connexion à Moodle PSL...")

        r = self.get(login_url)
        if not r:
            raise Exception("Impossible d'accéder à la page de login")

        # Si déjà connecté (rare), on continue directement
        if self.base_url in r.url and "login" not in r.url:
            log.info("✓ Déjà connecté")
            return True

        # Shibboleth, OAuth ou tout SSO non-CAS → login navigateur
    ...
    def _login_browser(self, login_url):
        """
        Ouvre une fenêtre Chrome, attend que l'utilisateur se connecte,
        puis transfère les cookies dans la session requests.
        """
        log.info("=" * 60)
        log.info("  CONNEXION MANUELLE REQUISE")
        log.info("  Une fenêtre Chrome va s'ouvrir.")
        log.info("  Connecte-toi à Moodle PSL (via Microsoft),")
        log.info("  puis la fenêtre se fermera automatiquement.")
        log.info("=" * 60)

        opts = ChromeOptions()
        opts.add_argument("--start-maximized")
        # Ne pas ajouter --headless : on veut que la fenêtre soit visible
        driver = webdriver.Chrome(options=opts)

        try:
            driver.get(login_url)

            # Attendre jusqu'à 3 minutes que l'utilisateur soit connecté
            # On détecte la connexion quand l'URL contient /my/ ou /course/
            log.info("En attente de connexion (3 min max)...")
            WebDriverWait(driver, 180).until(
                lambda d: (
                    "/my" in d.current_url or
                    "/course" in d.current_url or
                    "/dashboard" in d.current_url
                ) and "login" not in d.current_url
            )

            log.info(f"✓ Connecté ! URL détectée : {driver.current_url}")

            # Transférer TOUS les cookies du navigateur vers requests
...
def main():
    ...
    print("=" * 60)
    print("   MOODLE OFFLINE SCRAPER — PSL")
    print(f"   Site : {MOODLE_URL}")
    print("   Connexion : navigateur Chrome (Shibboleth/Microsoft)")
    if args.test:
        print("   [MODE TEST — 2 cours maximum]")
    print("=" * 60)
    print()
    print("  → Une fenêtre Chrome va s'ouvrir pour la connexion.")
    print("  → Connecte-toi normalement, puis le scraping démarre.")
    print()
